...
function M.setup(opts)
  ...
  vim.cmd("augroup yanil_git")
  vim.cmd("autocmd!")
  vim.cmd(
    [[autocmd BufWritePost * if empty(&buftype) | call luaeval('require("yanil/git").update()') | endif]]
  )
  vim.cmd("autocmd User FugitiveChanged lua require('yanil/git').update()")
  vim.cmd("augroup end")
end
...
function M.update(cwd)
  ...
  local status_callback =
    ...
    function(code, _signal, stdout, stderr)
      if code > 0 then
        api.nvim_echo({string.format("git status failed: %s", stderr)}, true, {err=true})
...
function M.apply_buf(bufnr)
  ...
  if vim.v.shell_error > 0 then
    api.nvim_echo({string.format("git apply failed: %s", output)}, true, {err=true})
    return
  end
